...
# config mlir
# build with clang
cmake -G Ninja ${top_dir_realpath}/external/llvm-project/llvm -DLLVM_ENABLE_PROJECTS=mlir -DLLVM_BUILD_EXAMPLES=ON -DLLVM_TARGETS_TO_BUILD="X86" -DCMAKE_BUILD_TYPE=DEBUG -DLLVM_ENABLE_ASSERTIONS=ON -DLLVM_ENABLE_LLD=ON -DCMAKE_C_COMPILER=clang-11 -DCMAKE_CXX_COMPILER=clang++-11 -DCMAKE_INSTALL_PREFIX=${top_dir_realpath}/mlir_build/install_dir -DLLVM_INSTALL_UTILS=ON
# build with gcc
